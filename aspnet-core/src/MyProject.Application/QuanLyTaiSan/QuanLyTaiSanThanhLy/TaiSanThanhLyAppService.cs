namespace MyProject.TaiSanThanhLy
{
    ...
    public class TaiSanThanhLyAppService : MyProjectAppServiceBase, ITaiSanThanhLyAppService
    {
        private readonly IRepository<ViewTaiSanThanhLy> vTaiSanThanhLyRespository;
        ...
        public TaiSanThanhLyAppService(
            IRepository<ViewTaiSanThanhLy> vTaiSanThanhLyRespository,
        ...
        {
            this.vTaiSanThanhLyRespository = vTaiSanThanhLyRespository;
        ...
        public async Task<PagedResultDto<ViewTaiSanThanhLy>> GetAllTaiSanThanhLyAsync(TaiSanThanhLyGetAllInputDto input)
        {
            #region Check null
            if (input == null)
            {
            ...
            if (!checkPermission)
            {
                var items = this.vTaiSanThanhLyRespository.GetAll().Where(w => phongBanCuaNguoiDung.Contains((int)w.PhongBanQuanLyId)).OrderByDescending(o => o.LastModificationTime).AsQueryable();
                var query = this.vTaiSanThanhLyRespository.GetAll().Where(w => 1 == 0);
                ...
                return new PagedResultDto<ViewTaiSanThanhLy>(totalCount, output);
            }
            else
            {
                var items = this.vTaiSanThanhLyRespository.GetAll().OrderByDescending(o => o.LastModificationTime).AsQueryable();
                var query = this.vTaiSanThanhLyRespository.GetAll().Where(w => 1 == 0);
        ...
        public async Task<int> CreateTaiSanThanhLy(PhieuTaiSanCreateInputDto input)
        {
            #region Check null
            if (input == null)
            {
            ...
            foreach (var ts in b)
            {
                ...
                {
                    noiDung = nguoiGui + " đã thanh lý " + ts.ListTS[0].TenTaiSan;
                }
                else if (ts.ListTS.Count > 1)
                {
                    noiDung = nguoiGui + " đã thanh lý " + ts.ListTS.Count + " tài sản";
                }

                await GlobalFunction.GuiThongBaoAsync(this.canhBaoRepository, this.toChucRespository, listUser, this.PermissionChecker, (int)this.AbpSession.UserId, ts.ToChucQuanLyId, noiDung, (int)GlobalConst.CanhBaoThongBao.ThanhLy);
            }

            return 0;
        }

        public async Task HoanTacTaiSanThanhLyAsync(EntityDto[] input)
        {
            #region Check null
            if (input == null)
            {
