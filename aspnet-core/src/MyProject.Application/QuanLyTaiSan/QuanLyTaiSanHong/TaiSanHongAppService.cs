namespace MyProject.TaiSanHong
{
    ...
    public class TaiSanHongAppService : MyProjectAppServiceBase, ITaiSanHongAppService
    {
        private readonly IRepository<ViewTaiSanHong> vTaiSanHongRespository;
        ...
        public TaiSanHongAppService(
            IRepository<ViewTaiSanHong> vTaiSanHongRespository,
        ...
        {
            this.vTaiSanHongRespository = vTaiSanHongRespository;
        ...
        public async Task<PagedResultDto<ViewTaiSanHong>> GetAllTaiSanHongAsync(TaiSanHongGetAllInputDto input)
        {
            #region Check null
            if (input == null)
            {
            ...
            if (!checkPermission)
            {
                var items = this.vTaiSanHongRespository.GetAll().Where(w => phongBanCuaNguoiDung.Contains((int)w.PhongBanQuanLyId)).OrderByDescending(o => o.LastModificationTime).AsQueryable();
                var query = this.vTaiSanHongRespository.GetAll().Where(w => 1 == 0);
                ...
                return new PagedResultDto<ViewTaiSanHong>(totalCount, output);
            }
            else
            {
                var items = this.vTaiSanHongRespository.GetAll().OrderByDescending(o => o.LastModificationTime).AsQueryable();
                var query = this.vTaiSanHongRespository.GetAll().Where(w => 1 == 0);
        ...
        public async Task<int> CreateTaiSanHong(PhieuTaiSanCreateInputDto input)
        {
            #region Check null
            if (input == null)
            {
            ...
            foreach (var ts in b)
            {
                ...
                {
                    noiDung = nguoiGui + " đã báo hỏng " + ts.ListTS[0].TenTaiSan;
                }
                else if (ts.ListTS.Count > 1)
                {
                    noiDung = nguoiGui + " đã báo hỏng " + ts.ListTS.Count + " tài sản";
                }

                await GlobalFunction.GuiThongBaoAsync(this.canhBaoRepository, this.toChucRespository, listUser, this.PermissionChecker, (int)this.AbpSession.UserId, ts.ToChucQuanLyId, noiDung, (int)GlobalConst.CanhBaoThongBao.BaoHong);
            }

            return 0;
        }

        public async Task HoanTacTaiSanHongAsync(EntityDto[] input)
        {
            #region Check null
            if (input == null)
            {
