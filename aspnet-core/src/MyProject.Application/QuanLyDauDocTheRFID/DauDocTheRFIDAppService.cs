namespace MyProject.QuanLyDauDocTheRFID
{
    using System;
    ...
    public class DauDocTheRFIDAppService : MyProjectAppServiceBase, IDauDocTheRFIDAppService
    {
        ...
        public DauDocTheRFIDAppService(
        ...
        public async Task<PagedResultDto<GetAllOutputDto>> GetAll(InputRFIDDto input)
        {
            ...
            filter = this.taiSanRepository.GetAll().Where(w => input.PhongBanSuDung.Contains((int)w.ToChucId))
                ...
                .Where(w => w.LoaiTaiSanId == (int)GlobalConst.LoaiTaiSanConst.DiDong && (w.TenTaiSan.Contains(input.TenTS) || input.TenTS == null) && (w.TenTaiSan.Contains(input.TenTS) || input.TenTS == null))
        ...
        public async Task<string> ImportFileExcel(string filePath)
        {
            ...
            // Không có dữ liệu
            if (data.Count <= 0 || data[0].Count != 10)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < data.Count; i++)
                {
                    ...
                    string ncc = GlobalFunction.RegexFormat(data[i][3]);
                    string hangSanXuat = GlobalFunction.RegexFormat(data[i][4]);
                    string nguyenGia = GlobalFunction.RegexFormat(data[i][5]);
                    string ngayMua = GlobalFunction.RegexFormat(data[i][6]);
                    string ngayHetHanBH = GlobalFunction.RegexFormat(data[i][7]);
                    string ngayHetHanSuDung = GlobalFunction.RegexFormat(data[i][8]);
                    string ghiChu = GlobalFunction.RegexFormat(data[i][9]);
                    ...
                    if (string.IsNullOrEmpty(error))
                    {
                        ...
                        {
                            TenTS = tenDauLoc,
                            LoaiTS = (int)GlobalConst.LoaiTaiSanConst.DiDong,
        ...
        {
            string fileName = string.Format("DauDocDiDong.xlsx");
            try
            {
                // _appFolders.DemoFileDownloadFolder : Thư mục chưa file mẫu cần tải
                // _appFolders.TempFileDownloadFolder : Không được sửa
        ...
        public async Task<FileDto> ExportToExcel(InputRFIDDto input)
        {
            ...
            // Tên file
            var fileName = string.Join(".", new string[] { "Danh sách đầu đọc di động", "xlsx" });
